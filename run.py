...
try:
    ...
    color_det = ColorRuntimeDetector(
        ...
        unknown_threshold=30.0,  # 元と同値に。調整可
        read_interval=0.02,
        moving_avg_n=1,
        distance_scale=1.0       # 元スクリプトと d を完全一致させる
    )
    try:
...
for _p in _profiles_path_candidates:
    try:
        ...
        if isinstance(prof, dict):
            ...
            # 代表的には "means", "stds" を持つ辞書のキーが色名
            # 並びは固定しない（JSONの順を尊重したい場合はそのまま）
        break
    except Exception:
        pass
